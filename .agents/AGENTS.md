...
2. Build and install locally:
   ...
   ```
   Re-run `meson install -C __build` after source edits to refresh the local install without wiping.
...
- `src/pages/generator.py`: Orchestrates generator flow with `GeneratorPage` (line 51).
- `src/pages/generator_wifi.py`: `GeneratorWiFiPage` (line 35) handles Wi-Fi network selection and QR generation.
- `src/pages/generator_starting.py`: Entry point page for the generator flow.
...
- Blueprint files live in `src/ui/generator/` (qr-code-page.blp, starting-page.blp, wifi-page.blp) and `src/ui/scanner/`.
